...
--
-- This is the single most reused query pattern in data engineering. Learn it
-- once here and you will write it every week for the rest of your career.

WITH ranked AS (
    SELECT
        o.*,
        ROW_NUMBER() OVER (
            PARTITION BY o.order_id                -- one group per business key
            ORDER BY o.updated_at DESC,            -- latest correction wins
                     o.order_row_id DESC           -- deterministic tie-break
        ) AS rn
    FROM orders AS o
)
SELECT
